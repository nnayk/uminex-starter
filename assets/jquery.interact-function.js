...
var AloReference = (function Alo($){ 
  ...
  Alothemes.SwitchRTL = function () {
      $(document).on('click', '.switch-rtl-ltr', function() {
          ...
          if(!$('#rtl-style').length){
              $('head').append('<link rel="stylesheet" id="rtl-style" href="//demo-morata-2.myshopify.com/cdn/shop/t/6/assets/style.rtl.css?1068" type="text/css" />');
          }
      });
  }();
      ...
      SortingCollections.prototype = $.extend({}, SortingCollections.prototype, {
          onLoad: function () {
              ...
              $control.each(function () {
                  ...
                  $this.closest('section').find('.laber-view-all a').attr('href', $(this).find('a.active').attr('href'));
              ...
              $control.on('click', 'a', function (e) {
                  ...
                  $this.closest('section').find('.laber-view-all a').attr('href', $this.attr('href'));
...
if (!customElements.get("product-options")) {
  class ProductOptions extends HTMLElement {
    ...
    onProcess(event) {
      ...
      if (labelSelected) {
        labelSelected.innerHTML = target.matches("option")
          ? target.value
      ...
      self.loadJSON($product, json, function (json) {
        ...
        $.each(json.variants, function () {
          ...
          if (
            current_values[position] == this.options[position] &&
            current_values.toString() === this.options.toString()
          ) {
            update_variant = this;
            return false;
          }
    ...
    updateAvailability($product, data) {
      ...
      if ($availability.length) {
        var html = "",
          vStr = "<span><b>{{ count }}</b> Products</span>";
...
if (!customElements.get("buy-now")) {
  class BuyNow extends HTMLElement {
    ...
    load() {
      var self = this,
        buyitnow = this.querySelector('.group-button-buyitnow');
      if (!buyitnow) return;
      var paymentButton = this.querySelector('.shopify-payment-button__button'),
          condition = this.querySelector('input[type="checkbox"]');
      if(condition && paymentButton){
          if(!condition.checked) paymentButton.setAttribute('disabled', true);
          condition.addEventListener('change', function(event) {
              if (event.currentTarget.checked) {
                  buyitnow.classList.remove('disabled');
                  self.querySelector('.shopify-payment-button__button').removeAttribute('disabled');
              } else {
                  buyitnow.classList.add('disabled');
                  self.querySelector('.shopify-payment-button__button').setAttribute('disabled', true);
              }
          });
      }
    }
  ...
  Alothemes.TermsConditions = function () {
      var $conditionsCart = $('.conditions_form_cart');
      if (!$conditionsCart.length) return;
      $formCart = $conditionsCart.closest('form');
      $btncheckout = $formCart.find('.btn_checkout');
      $btncheckout.prop("disabled", true);
      $conditionsCart.change(function () {
          if ($conditionsCart.is(":checked")) {
          ...
          checkProductStatus: function ($products) {
              ...
              $.each($products, function () {
                  ...
                  if(!handle){
                      var dataJson = $product.find('.data-json-product'),
                          jsonProduct = dataJson.length ? JSON.parse(dataJson.html()) : $product.data('json-product');
                      if(jsonProduct) handle = jsonProduct.handle;                      
                  }
      ...
      Menu.prototype = $.extend({}, Menu.prototype, {
          ...
          allCategories: function () {
              var $vmenu = $('.vertical_menu'),
                  moreCat = $vmenu.find('.js-all-cat'),
                  num = moreCat.data('numb'),
                  allItems = $vmenu.find('.lazy_vertical_menu > li').not('.js-all-cat');
              if($vmenu.hasClass('init')){
                return;
              } else {
                $vmenu.addClass('init');
              }
  ...
  $(function(){ 
      ...
      aloShopify = function () {
          return {
              ...
              searchAjax: function () {
                  ...
                  $(document).on('mouseover', ".aloSearch", function () {
                      $(this).find(".js_iput_search").focus();
                      Alothemes.ProductCurrency.update();
                  });
                  /* add search mask */
              ...
              addItemToCart: function ($formData, $this) {
                  ...
                  $body.trigger('beforeAjax:addToCart', $formData);
                      fetch(Shopify.routes.root + 'cart/add.js', {
                        ...
                        .then((response) => {
                          var originalText = $this.find('.text').html();
                          if (response.status) {
                            /* Fail */
                            ...
                            setTimeout(function () {
                                if(!$this.is(':disabled')){
                                    $this.removeClass('added').find('.text').html(originalText);
                                }
                            }, 1000);
              ...
              ajaxCart: function () {
                      ...
                      if($body.hasClass('template-product')){
                          ...
                          if( singleProduct.length ){
                              if(disableAutoSelect){
                                  var options = singleProduct.find('.cms-option-item');
                                  if(options.length && !options.find('.active').length){
                  ...
                  $(document).on('click', ".js-remove-item", function (event) {
                      ...
                      document.dispatchEvent(new Event('startLoading'));
                      ...
                      fetch(Shopify.routes.root + 'cart/change.js', {
                        ...
                        .then(response => {
                            document.dispatchEvent(new Event('endLoading'));
                      ...
                      fetch(Shopify.routes.root + 'cart/change', {
                          ...
                          return response.text();
                            ...
                            if (response.description || response.errors) {
                              ...
                              document.dispatchEvent(new CustomEvent('drawer:Minicart', {detail:{response: response}}));
              ...
              miniCart: function () {
                  document.addEventListener("drawer:Minicart", function (event) {
                     var $minicart = $('#js_cart_popup'),
                      ...
                      if(data.hasOwnProperty('response') && data.response.sections){
                        ...
                        if(sections.hasOwnProperty('minicart') && sections.minicart){
                              var cart = $(sections.minicart);
                              if(cart.find('#MainContent').length) cart = cart.find('#MainContent');
                              $minicart.html(cart);
                              var data = cart.find('#json-data-cart');
                              data = JSON.parse(data.text());
                              $(".js-total-price, .info-total .total").html(Shopify.formatMoney(data.total_price, theme.moneyFormat)).find('.money').css('opacity', '0').delay(500).fadeTo(50, 1);
                              $(".info-subtotal .subtotal").html(Shopify.formatMoney(data.items_subtotal_price, theme.moneyFormat)).find('.money').css('opacity', '0').delay(500).fadeTo(50, 1);
                  ...
                  $('body').on('update:miniCart ajax:addToCart ajax:updateCart ajax:deleteCart', function (event) {
                      var data = event.detail || {};
                      if(data.hasOwnProperty('response') && data.response.sections){
                        var sections = data.response.sections;
                        if(sections.hasOwnProperty('minicart') && sections.minicart){
                            document.dispatchEvent(new CustomEvent('drawer:Minicart', {detail:data}));
                        }
                      }else{
                        fetch(Shopify.routes.root + 'cart?view=minicart')
                            .then((response) => response.text())
                            .then((responseText) => {
                                document.dispatchEvent(new CustomEvent('drawer:Minicart', {
              ...
              mainCart: function() {
                  ...
                  $('body').on('ajax:addToCart ajax:updateCart ajax:deleteCart drawer:Minicart', function (e, $data) {
              ...
              $('.cms-countdown').not('.init').each(function () {
                  var $counter = $(this),
                      ...
                      mm     = Number($counter.data('m').toString().replace("mm", new Date().getMonth() + 1)),
                      dd     = Number($counter.data('d').toString().replace("dd", new Date().getDate() + 1));
                  var austDay = new Date(year, mm -1 , dd, $counter.data('h'), $counter.data('i'), $counter.data('s')); // the month is 0-indexed
                  $counter.countdown({
                      until: austDay,
                      labels: labels,
                      layout: layout,
          ...
          PromotionPopup: function () {
              ...
              if (!popupPromtion.length || $(window).width() < 1025 || $.cookie(popupType) == 'shown') return;
              ...
              setTimeout(function () {
                  ...
                  $.magnificPopup.open({
                      ...
                      callbacks: {
                          ...
                          close: function () {
                              // $.cookie(popupType, 'shown', { expires: stt.day_next, path: '/' });
                          }
                      }
                  });
              }, time);
